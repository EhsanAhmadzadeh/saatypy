...
def test_pairwise_comparison_edge_cases():
    ...
    # 2x2 matrix with extreme values (direct matrix accepts any positive values)
    ...
    # Almost inconsistent reciprocal -> now raises ConsistencyError
...
def test_anp_edge_cases():
    ...
    # Empty cluster
    model = ANPBuilder().add_cluster("empty", []).add_alternatives(["a1"]).build()

    # Single element clusters
    model = ANPBuilder().add_cluster("single", ["node1"]).add_alternatives(["a1"]).build()

    # Equal cluster weights
    model.set_cluster_weights({"single": 0.5, "alternatives": 0.5})
...
def test_limiter_edge_cases():
    ...
    M = np.array([[0.99, 0.02], [0.01, 0.98]])
    # Should still converge
    limit = power_limiter.limit(M, max_iter=10000)
    assert np.allclose(limit[:, 0], limit[:, 1])  # All columns equal at the limit
    ...
    # Test damped vs power method convergence (should agree on limit)
...
def test_invalid_model_operations():
    ...
    model = AHPBuilder().add_criteria(["c1"]).add_alternatives(["a1"]).build()
    with pytest.raises(ValueError):
        model.alternative_priorities()

    # ANP: Try to build supermatrix without cluster weights
    model = ANPBuilder().add_cluster("c1", ["n1"]).add_alternatives(["a1"]).build()
    with pytest.raises(ValueError):
        model.alternative_priorities()
...
def test_numerical_stability():
    ...
    # Test with very small/large reciprocal values in a direct matrix
